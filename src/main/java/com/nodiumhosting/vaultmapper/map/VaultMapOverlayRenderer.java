...
public class VaultMapOverlayRenderer {
    ...
    public static void eventHandler(RenderGameOverlayEvent.Post event) {
        ...
        if (VaultMap.currentRoom != null) {
            ...
            bufferBuilder.vertex(triag.get(0) + mapX + (3 * scaleFactor), triag.get(1) + mapZ, 0).color(parseColor(ClientConfig.POINTER_COLOR.get())).endVertex();
            bufferBuilder.vertex(triag.get(2) + mapX + (3 * scaleFactor), triag.get(3) + mapZ, 0).color(parseColor(ClientConfig.POINTER_COLOR.get())).endVertex();
            bufferBuilder.vertex(triag.get(4) + mapX + (3 * scaleFactor), triag.get(5) + mapZ, 0).color(parseColor(ClientConfig.POINTER_COLOR.get())).endVertex();
            bufferBuilder.end();
    ...
    private static ArrayList<Float> getRotatedTriangle() { // returns three points that make a rotated triangle when added with mapx,z
        double x1 = -3 * scaleFactor;
        double y1 = -2 * scaleFactor;
        double x2 = -3 * scaleFactor;
        double y2 = 2 * scaleFactor;
        double x3 = 3 * scaleFactor;
        double y3 = 0;

        double cx = -3 * scaleFactor; // centers to rotate about
        double cy = 0;
    ...
    public static void onWindowResize() {
        ...
        float mapSize = (w * 0.25f);

        //mapRoomWidth = mapSize / 49;
        scaleFactor = ((float) ClientConfig.MAP_SCALE.get() / 10);
        mapRoomWidth = (mapSize / 49) * scaleFactor;
