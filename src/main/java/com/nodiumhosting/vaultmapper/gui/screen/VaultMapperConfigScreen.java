...
public class VaultMapperConfigScreen extends Screen {
    ...
    protected void init() {
        ...
        Button mapEnabledButton = new Button(this.width / 2 - 100, getScaledY(y++ + 0.25f), elWidth, Math.min((getScaledY(1) / 3) * 2, 20), new TextComponent("Map Enabled: " + ClientConfig.MAP_ENABLED.get()), button -> {
        ...
        Button webMapEnabledButton = new Button(this.width / 2 - 100, getScaledY(y++), elWidth, Math.min((getScaledY(1) / 3) * 2, 20), new TextComponent("WebMap Enabled: " + ClientConfig.WEBMAP_ENABLED.get()), button -> {
...
//            }
        ...
        Slider mapScale = new Slider(this.width / 2 + 10, getScaledY(y++), "", ClientConfig.MAP_SCALE.get(), 30, 3, mapScaleGetter, width, elHeight, 10);
        this.addRenderableWidget(mapScale);

        Slider PCCutoff = new Slider(this.width / 2 + 10, getScaledY(y), "", ClientConfig.PC_CUTOFF.get(), 30, 4, PCCutoffGetter, width, elHeight, 10);
        this.addRenderableWidget(PCCutoff);

        Checkbox playerCentric = new Checkbox(this.width / 2 + elWidthColor + 5 + 10 + elHeight + 5 - 2, getScaledY(y++) - 3, 20, 20, new TextComponent("Player Centric Rendering"), ClientConfig.PLAYER_CENTRIC_RENDERING.get());
        this.addRenderableWidget(playerCentric);

        EditBoxReset mapXOffset = new EditBoxReset(this.font, this.width / 2 + 10, getScaledY(y++), width, elHeight, new TextComponent("MAP_X_OFFSET"), "0");
        mapXOffset.setValue(ClientConfig.MAP_X_OFFSET.get().toString());
        this.addRenderableWidget(mapXOffset);

        EditBoxReset mapYOffset = new EditBoxReset(this.font, this.width / 2 + 10, getScaledY(y++), width, elHeight, new TextComponent("MAP_Y_OFFSET"), "0");
        ...
        Slider mapXAnchor = new Slider(this.width / 2 + 10, getScaledY(y++), "", ClientConfig.MAP_X_ANCHOR.get(), 4, 0, anchorGetterX, width, elHeight, 4);
        ...
        Slider mapYAnchor = new Slider(this.width / 2 + 10, getScaledY(y), "", ClientConfig.MAP_Y_ANCHOR.get(), 4, 0, anchorGetterY, width, elHeight, 4);
        this.addRenderableWidget(mapYAnchor);

        ColorPicker colorPicker = new ColorPicker(Clamp.clamp(this.width / 2 + 200, 0, this.width - 200), getScaledY(y++), 200, 200, parseColor("#000000"), button -> {
        });
        colorPicker.visible = false;
        this.addRenderableWidget(colorPicker);

        EditBoxReset pointerColor = new EditBoxReset(this.font, this.width / 2 + 10, getScaledY(y), elWidthColor, elHeight, new TextComponent("POINTER_COLOR"), "#00FF00");
        pointerColor.setValue(ClientConfig.POINTER_COLOR.get());
        this.addRenderableWidget(pointerColor);
        ColorButton pointerColorPicker = new ColorButton(this.width / 2 + elWidthColor + 5 + 10, getScaledY(y++), elHeight, elHeight, parseColor(ClientConfig.POINTER_COLOR.get()), button -> {
        ...
        EditBoxReset roomColor = new EditBoxReset(this.font, this.width / 2 + 10, getScaledY(y), elWidthColor, elHeight, new TextComponent("ROOM_COLOR"), "#0000FF");
        roomColor.setValue(ClientConfig.ROOM_COLOR.get());
        this.addRenderableWidget(roomColor);
        ColorButton roomColorPicker = new ColorButton(this.width / 2 + elWidthColor + 5 + 10, getScaledY(y++), elHeight, elHeight, parseColor(ClientConfig.ROOM_COLOR.get()), button -> {
        ...
        EditBoxReset startRoomColor = new EditBoxReset(this.font, this.width / 2 + 10, getScaledY(y), elWidthColor, elHeight, new TextComponent("START_ROOM_COLOR"), "#FF0000");
        startRoomColor.setValue(ClientConfig.START_ROOM_COLOR.get());
        this.addRenderableWidget(startRoomColor);
        ColorButton startRoomColorPicker = new ColorButton(this.width / 2 + elWidthColor + 5 + 10, getScaledY(y++), elHeight, elHeight, parseColor(ClientConfig.START_ROOM_COLOR.get()), button -> {
        ...
        EditBoxReset markedRoomColor = new EditBoxReset(this.font, this.width / 2 + 10, getScaledY(y), elWidthColor, elHeight, new TextComponent("MARKED_ROOM_COLOR"), "#FF00FF");
        markedRoomColor.setValue(ClientConfig.MARKED_ROOM_COLOR.get());
        this.addRenderableWidget(markedRoomColor);
        ColorButton markedRoomColorPicker = new ColorButton(this.width / 2 + elWidthColor + 5 + 10, getScaledY(y++), elHeight, elHeight, parseColor(ClientConfig.MARKED_ROOM_COLOR.get()), button -> {
        ...
        EditBoxReset inscriptionRoomColor = new EditBoxReset(this.font, this.width / 2 + 10, getScaledY(y), elWidthColor, elHeight, new TextComponent("INSCRIPTION_ROOM_COLOR"), "#FFFF00");
        inscriptionRoomColor.setValue(ClientConfig.INSCRIPTION_ROOM_COLOR.get());
        this.addRenderableWidget(inscriptionRoomColor);
        ColorButton inscriptionRoomColorPicker = new ColorButton(this.width / 2 + elWidthColor + 5 + 10, getScaledY(y), elHeight, elHeight, parseColor(ClientConfig.INSCRIPTION_ROOM_COLOR.get()), button -> {
        ...
        Checkbox showInscription = new Checkbox(this.width / 2 + elWidthColor + 5 + 10 + elHeight + 5 - 2, getScaledY(y++) - 3, 20, 20, new TextComponent("Inscriptions"), ClientConfig.SHOW_INSCRIPTIONS.get());
        this.addRenderableWidget(showInscription);

        EditBoxReset omegaRoomColor = new EditBoxReset(this.font, this.width / 2 + 10, getScaledY(y), elWidthColor, elHeight, new TextComponent("OMEGA_ROOM_COLOR"), "#55FF55");
        omegaRoomColor.setValue(ClientConfig.OMEGA_ROOM_COLOR.get());
        this.addRenderableWidget(omegaRoomColor);
        ColorButton omegaRoomColorPicker = new ColorButton(this.width / 2 + elWidthColor + 5 + 10, getScaledY(y++), elHeight, elHeight, parseColor(ClientConfig.OMEGA_ROOM_COLOR.get()), button -> {
        ...
        EditBoxReset challengeRoomColor = new EditBoxReset(this.font, this.width / 2 + 10, getScaledY(y), elWidthColor, elHeight, new TextComponent("CHALLENGE_ROOM_COLOR"), "#F09E00");
        challengeRoomColor.setValue(ClientConfig.CHALLENGE_ROOM_COLOR.get());
        this.addRenderableWidget(challengeRoomColor);
        ColorButton challengeRoomColorPicker = new ColorButton(this.width / 2 + elWidthColor + 5 + 10, getScaledY(y++), elHeight, elHeight, parseColor(ClientConfig.CHALLENGE_ROOM_COLOR.get()), button -> {
        ...
        int roomIconsCheckboxY = getScaledY(y) + (getScaledY(1) / 2) - 2;
        Checkbox showRoomIcons = new Checkbox(this.width / 2 + elWidthColor + 5 + 10 + elHeight + 5 - 2, roomIconsCheckboxY, 20, 20, new TextComponent("Room Icons"), ClientConfig.SHOW_ROOM_ICONS.get());
        this.addRenderableWidget(showRoomIcons);

        EditBoxReset syncServer = new EditBoxReset(this.font, this.width / 2 - 70, getScaledY(y), elWidthColor + 80, elHeight, new TextComponent("SYNC_SERVER"), "wss://vmsync.ndmh.xyz");
        ...
        Button enableSyncButton = new Button(this.width / 2 + elWidthColor + 5 + 10, getScaledY(y++), elHeight, elHeight, ClientConfig.SYNC_ENABLED.get() ? enabledText : disabledText, button -> {
        ...
        EditBoxReset syncColor = new EditBoxReset(this.font, this.width / 2 + 10, getScaledY(y), elWidthColor, elHeight, new TextComponent("SYNC_COLOR"), Util.RandomColor());
        syncColor.setValue(ClientConfig.SYNC_COLOR.get());
        this.addRenderableWidget(syncColor);
        ColorButton syncColorPicker = new ColorButton(this.width / 2 + elWidthColor + 5 + 10, getScaledY(y++), elHeight, elHeight, parseColor(ClientConfig.SYNC_COLOR.get()), button -> {
        ...
        Button saveButton = new Button(this.width / 2 - 100, getScaledY(y++), 200, Math.min((getScaledY(1) / 3) * 2, 20), new TextComponent("Save"), button -> {
            try {
        ...
        Button resetButton = new Button(this.width / 2 - 100, getScaledY(y++ - 0.25f), 200, Math.min((getScaledY(1) / 3) * 2, 20), new TextComponent("Reset"), button -> {
    ...
    public void render(PoseStack pose, int mouseX, int mouseY, float partialTick) {
        ...
        if (webMapEnabled != ClientConfig.WEBMAP_ENABLED.get()) {
            ...
            this.font.draw(pose, "Changes require a game restart", this.width - 8 - this.font.width("Changes require a game restart"), 8, 0xFFFF0000);
        }

        // labels
        int y = 2;
        int offsetY = getScaledY(1) / 8;
        this.font.draw(pose, "Map Scale", this.width / 2 - 110, getScaledY(y++) + offsetY, 0xFFFFFFFF);
        this.font.draw(pose, "Player Centric Cutoff", this.width / 2 - 110, getScaledY(y++) + offsetY, 0xFFFFFFFF);
        this.font.draw(pose, "Map X Offset", this.width / 2 - 110, getScaledY(y++) + offsetY, 0xFFFFFFFF);
        this.font.draw(pose, "Map Y Offset", this.width / 2 - 110, getScaledY(y++) + offsetY, 0xFFFFFFFF);
        this.font.draw(pose, "Map X Anchor", this.width / 2 - 110, getScaledY(y++) + offsetY, 0xFFFFFFFF);
        this.font.draw(pose, "Map Y Anchor", this.width / 2 - 110, getScaledY(y++) + offsetY, 0xFFFFFFFF);
        this.font.draw(pose, "Pointer Color", this.width / 2 - 110, getScaledY(y++) + offsetY, 0xFFFFFFFF);
        this.font.draw(pose, "Room Color", this.width / 2 - 110, getScaledY(y++) + offsetY, 0xFFFFFFFF);
        this.font.draw(pose, "Start Room Color", this.width / 2 - 110, getScaledY(y++) + offsetY, 0xFFFFFFFF);
        this.font.draw(pose, "Marked Room Color", this.width / 2 - 110, getScaledY(y++) + offsetY, 0xFFFFFFFF);
        this.font.draw(pose, "Inscription Room Color", this.width / 2 - 110, getScaledY(y++) + offsetY, 0xFFFFFFFF);
        this.font.draw(pose, "Omega Room Color", this.width / 2 - 110, getScaledY(y++) + offsetY, 0xFFFFFFFF);
        this.font.draw(pose, "Challenge Room Color", this.width / 2 - 110, getScaledY(y++) + offsetY, 0xFFFFFFFF);
        this.font.draw(pose, "Ore Room Color", this.width / 2 - 110, getScaledY(y++) + offsetY, 0xFFFFFFFF);
        this.font.draw(pose, "VMSync", this.width / 2 - 110, getScaledY(y++) + offsetY, 0xFFFFFFFF);
        this.font.draw(pose, "Sync Color", this.width / 2 - 110, getScaledY(y++) + offsetY, 0xFFFFFFFF);
